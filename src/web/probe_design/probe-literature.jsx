...
function Literature({ mainRq, selectedBranches, onBack, onContinue, goTo }) {
  ...
  const [liveLit, setLiveLit] = useState({}); // letter → { stateOfArt, similar, gaps } from /api/probe/literature
  const tabRef = useRef(null);

  // Stage advancement timing — also kicks off real LLM calls in
  // parallel for each selected RQ. The visual stage progression
  // (search → read → synthesize) runs on the same timer regardless
  // so the UX stays consistent whether the API is fast or slow.
  useEffect(() => {
    const t1 = setTimeout(() => setStage(1), 900);
    const t2 = setTimeout(() => setStage(2), 1900);
    const t3 = setTimeout(() => setStage(3), 3100);
    // Fire one call per selected RQ — they run in parallel.
    selectedBranches.forEach((rq) => {
      fetch('/api/probe/literature', {
        method: 'POST',
        ...
        .then((data) => {
          if (data && data.block) {
            setLiveLit((prev) => ({ ...prev, [rq.letter]: data.block }));
          }
        })
        .catch(() => { /* fallback to stock content already in scope */ });
    });
  ...
  // Prefer the live LLM-generated block when available; fall back to
  // the stock library entry by angle. Shape is the same either way:
  // { stateOfArt | soa, similar | papers, gaps }.
  const live = liveLit[active.letter];
  const lit = live
    ? { soa: live.stateOfArt || '', papers: live.similar || [], gaps: live.gaps || [] }
    : (stockLiterature[active.angle] || stockLiterature.custom);
