...
function Methodology({ mainRq, selectedBranches, onBack, onContinue, goTo }) {
  ...
  // list and update once the LLM responds. The visual stream effect
  // ticks regardless so the screen feels active.
  //
  // The LLM returns `coverage: { A: 'core', B: 'partial', ... }` and
  // `weeks: 12`. The renderer expects `components: [{name, addresses}]`
  // and `duration: '12w'`. We translate here so the existing render
  // path works without a rewrite.
  const COV_VALUE = { core: 2, partial: 1, none: 0 };
  const adaptDesign = (c) => ({
    id: c.id || 'd' + Math.random().toString(36).slice(2, 7),
    name: c.name || 'Untitled design',
    summary: c.summary || '',
    arc: c.arc || '',
    duration: c.duration || (c.weeks ? `${c.weeks}w` : '— weeks'),
    components: Array.isArray(c.components) && c.components.length
      ? c.components
      : [{
          name: 'integrated approach',
          addresses: rqs.map((r) => {
            const v = (c.coverage && c.coverage[r.letter]) || 'partial';
            return COV_VALUE[v] ?? 1;
          }),
        }],
  ...
  useEffect(() => {
    ...
    setLiveDesigns(null);
    const ivs = [];
    fetch('/api/probe/methodology', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ premise: mainRqLocal, rqs }),
    })
  ...
  const draftPlan = () => {
    ...
    if (!d) return;
    // Optimistic: show the stock plan immediately so the user sees
    // something. Then call /api/probe/plan and replace once the LLM
    // returns a real one.
    const stub = buildPlan(d, rqs, planNotes);
    setPlan(stub);
    fetch('/api/probe/plan', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ design: d, premise: mainRqLocal }),
    })
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
        if (data && data.plan && Array.isArray(data.plan.phases) && data.plan.phases.length) {
          setPlan({
            phases: data.plan.phases.map((p, i) => ({
              id: p.id ?? `p${i}`,
              name: p.name ?? `phase ${i + 1}`,
              weeks: p.weeks ?? 1,
              detail: p.detail ?? '',
            })),
            deliverables: data.plan.deliverables || stub.deliverables,
            recruitment: data.plan.recruitment || stub.recruitment,
            timeline: data.plan.totalWeeks ? `${data.plan.totalWeeks} weeks` : stub.timeline,
            risks: data.plan.risks || stub.risks,
            notes: planNotes || stub.notes,
          });
        }
      })
      .catch(() => { /* keep the stub */ });
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <Crumb steps={[
        ...
        ['methodology', palette.ink, 'methodology'],
      ]} onStepClick={goTo}
      right={
