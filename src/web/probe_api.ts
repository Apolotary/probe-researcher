...
export function mountProbeApi(app: express.Express): void {
  ...
  // Sonnet-vs-Opus disagreement audit comparison.
  //
  // Round-N evaluator panel flagged that the "smaller models would
  // fail" claim is asserted but not shown. This endpoint surfaces a
  // pre-computed Sonnet 4.6 audit — generated by
  // scripts/regen_audit_sonnet.ts against the same review panel as
  // the bundled Opus 4.7 audit — so the review screen can render
  // them side-by-side. Returns 404 when the comparison file isn't
  // present so the UI can fall back gracefully.
  router.get('/audit-compare', async (_req, res) => {
    try {
      const fs = await import('node:fs');
      const path = await import('node:path');
      const root = (await import('../util/paths.js')).projectRoot();
      const filePath = path.join(root, 'assets', 'demos', 'focus-rituals-sonnet-audit.json');
      if (!fs.existsSync(filePath)) {
        res.status(404).json({ error: 'no comparison cached',
          hint: 'run `npx tsx scripts/regen_audit_sonnet.ts` to generate' });
        return;
      }
