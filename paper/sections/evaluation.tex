...
Probe is not a benchmark system. It produces one structured artifact per research premise, and the most honest evaluation asks whether that artifact is the one a researcher would actually use as a starting point. We report three partial evaluations, each explicit about what it does and does not support.
...
We also attempted a third benchmark in a creativity-support domain (a tool that offers poets alternative line endings during drafting) as a stress test of the out-of-domain pattern-firing behavior. That run's results are discussed inline where they are available.
...
The pattern that emerges is: the deep audit's value is not in finding different things from the shallow audit; it is in measuring the same things precisely enough that the researcher can make a revision decision from the report alone.
