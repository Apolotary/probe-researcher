...
The capture-risk audit fired sixteen pattern findings per branch. Branch A's audit fired \texttt{agency.auto\_decides\_consequential\_step} at minus-two: the proposed automated audit pipeline routes flagged hierarchy violations into a remediation queue without a human-in-the-loop review step. The model's reasoning: the intervention silently becomes a policy, ``what we flag, we change,'' where the decision to remediate is a consequential human-scale choice being delegated to pattern matching. Verdict: \texttt{BLOCKED}. The branch emits \texttt{WORKSHOP\_NOT\_RECOMMENDED.md} and does not advance.
...
Branch B's audit returned \texttt{REVISION\_REQUIRED}: two minus-one findings (around legibility of the ranking mechanism and around the audit's coverage of the debrief card) but no minus-two. The branch advances to adversarial review.

\subsection{Stage 7: three reviewers with three distinct objections}

The methodologist reviewer's first line is the announcement-duration confound:

\begin{quote}
The AI disclosure string is 18 words and $\sim$6 seconds of speech; the human-framed string is 5 words and $\sim$2 seconds. This confound is baked into the stimuli, and every behavioral dependent variable the study measures will be mechanically contaminated by announcement length rather than disclosure content.
\end{quote}

We did not see this when reading the prototype spec. Neither did the capture-risk auditor. The reviewer's recommendation is \texttt{major\_revision}. Six total criticisms, each attached to a quoted evidence span from the prototype spec or the simulated walkthrough.
