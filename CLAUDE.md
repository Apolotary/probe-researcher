...
- `src/web/probe_api.ts` — express endpoints that wrap `probe_calls.ts` for the web frontend. Each stage endpoint goes through a `served()` helper that short-circuits to the active demo's saved slice (with synthetic latency) when `probe_demo.isReplaying()` is true, and falls back to a shaped-but-empty payload (`emptyPayloadFor(stage)`) when the cache is missing AND the live LLM call fails — so a judge with no API key never sees a 500 mid-replay.
- `src/web/probe_demo.ts` — disk persistence for saved demos under `~/.config/probe/demos/<slug>.json` (user-saved, takes priority) and `assets/demos/` (bundled with the repo, judge-friendly fallback). `slice(stage, args)` extracts a per-stage payload from the saved state to mirror what the live endpoint would have returned. Bundled `focus-rituals.json` is the gold demo wired to the sidebar's "▶ replay sample run" button.
